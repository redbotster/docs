...
- **LangSmith Deployment usage** (`kind=langsmith_deployments`) — nodes executed, agent runs, and agent uptime for [LangSmith Deployment](/langsmith/billing#langsmith-deployment) (formerly LangGraph Platform).
...
3. Switch between the **LangSmith Traces** and **LangSmith Deployments** sub-tabs to view each domain. The active sub-tab is reflected in the URL (`?tab=traces` or `?tab=deployments`) so the page can be bookmarked and reloads land on the same view.
...
Both kinds aggregate from daily-rollup tables. The endpoint normalizes the window to whole days at the API layer:

- `start_time` is floored to its day's UTC midnight.
- `end_time` is ceiled to the next UTC midnight (no-op when already at midnight).
...
Data collection for Deployment usage began once the rollup job started running (see [self-hosted changelog](/langsmith/self-hosted-changelog) for the LangSmith version that enables it).
...
Cells whose value would start with `=`, `+`, `-`, `@`, tab, or carriage-return are tab-prefixed to neutralize spreadsheet formula injection (CWE-1236).
